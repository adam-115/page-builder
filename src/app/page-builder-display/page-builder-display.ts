...
export class PageBuilderDisplay implements OnInit {
  ...
  gridItems: FormElement[] = [
    { id: undefined, uiid: 1, customStyle: '', defaultValue: 'dfsdfsd', name: 'firstName', type: 'text', required: true, label: 'First Name', rowStart: 1, rowEnd: 2, colStart: 1, colEnd: 6, justify: 'end', align: 'start' },
    { id: undefined, uiid: 2, name: 'lastName', type: 'text', required: true, label: 'Last Name', rowStart: 1, rowEnd: 2, colStart: 6, colEnd: 11, justify: 'center', align: 'start' },
  ...
  */
  getGridItemClasses(item: FormElement): string {
    ...
    const baseClasses = 'w-full h-full flex flex-col  z-10 relative';
    ...
    if (item.justify) {
      ...
      justifyClass = (item.type === 'submit' || item.type === 'cancel') ? 'justify-end' : 'justify-end';
    }
    ...
    let borderClass = '';
    if (this.selectedFormElement?.uiid === item.uiid) {
  ...
  selectItem(item: FormElement): void {
    ...
    if (this.selectedFormElement != null && this.selectedFormElement?.uiid === item.uiid) {
  ...
  */
  ...
  createOrUpdateFormElement(formElement: FormElement): void {
    ...
    if (formElement.uiid === null || formElement.uiid === undefined) {
      alert("creation");
      formElement.uiid = this.nextUIId++;
      // 1. AJOUT (Create)
      ...
      const found = updatedGridItems.some(fE => fE.uiid === formElement.uiid);

      if (found) {
        this.gridItems = updatedGridItems;
        console.log('Élément mis à jour (immuable) :', formElement.uiid);
      } else {
  ...
  deleteFormElement(formElement: FormElement) {
    alert("start delete ");
    this.gridItems = this.gridItems.filter(fe => fe.uiid !== formElement.uiid);
  }


  onSaveGridConfig() {
    alert("save grid config not implemented yet" );
   let pageConfig:PageConfig = {
      id:null,
      gridNumberOfCols:this.gridNumberOfCols,
      gridNumberOfRows:this.gridNumberOfRows,
      gridRowHighInPX :this.gridRowHighInPX,
      gridCustomStyle:this.gridCustomStyle,
      formElements:this.gridItems,
      order:1,
      step:1,
    };
    this.pageConfigService.create(pageConfig).subscribe({
      next: (createdConfig) => {
        alert("Page configuration saved with ID: " + createdConfig.id);
      },
      error: (err) => {
        alert("Error saving page configuration: " + err.message);
